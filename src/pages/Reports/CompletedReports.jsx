...
import CompletedReportChart from '../../components/chartReports/CompletedReportChart';
...
const CompletedReports = () => {
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: "Completed Report",
    });
    

    const { completedReport, getCompletedReport, isLoading, error } = useAuthStore();

    useEffect(() => {
        const fetchData = async () => {
            try {
                await getCompletedReport(filter);
            } catch (error) {
    ...
    const completedReportData = completedReport?.tableData || []; // FIX: correctly map the "table" from API response
        ...
        <div className="flex flex-col space-y-6 px-4 pt-0 pb-8">
            ...
            <div ref={printRef}>
                ...
                <div className="w-full flex justify-center items-center print:pt-40">
                    <ChartBoxReport>
                        <CompletedReportChart filter={filter} />
                    </ChartBoxReport>
                </div>

                {/* Table Section */}
                <div className="bg-white rounded-lg shadow-md p-6 print:p-0 print:w-full">
                    <h2 className="text-lg font-semibold text-blue-800 mb-4 text-center">Completed Reservation Table</h2>

                    {isLoading ? (
                        <p className="text-blue-600">Loading...</p>
                    ) : error ? (
                        <p className="text-red-500">{error}</p>
                    ) : completedReportData.length === 0 ? (
                        <p className="text-gray-500 text-center">No completed reservations available for the selected range.</p>
                    ) : (
                        <div className="w-full">
                            <table className="w-full table-auto border border-gray-200 rounded-lg">
                                ...
                                <tbody>
                                    {completedReportData.map((reservation) => (
